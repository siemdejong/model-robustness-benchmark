...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache-dir",
        nargs="?",
        default=os.path.join(get_data_home(), "mnist_classifier_cache"),
        type=str,
        help="Directory to cache trained classifiers.",
    )
    parser.add_argument(
        "--inference-only",
        action="store_true",
        help="Skip training and use cached classifiers for inference only.",
    )
    ...
    if args["inference_only"]:
        print("Running Inference (using cached classifiers)")
    else:
    ...
    for name in sorted(args["classifiers"]):
        ...
        if args["inference_only"]:
            ...
            if not os.path.exists(cache_path):
                print(f"Skipping {name}: no cached model found at {cache_path}")
                continue
            print("Loading %s ... " % name, end="")
            estimator = load(cache_path)
            train_time[name] = 0.0
        else:
